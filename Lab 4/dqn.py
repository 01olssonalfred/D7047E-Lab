...
if __name__ == "__main__":
    ...
    for global_step in range(params.total_timesteps):
        ...
        if random.random() < epsilon:
            actions = np.array([envs.single_action_space.sample()]) # Exploration
        else:
            q_values = q_network(torch.tensor(obs, device=device))
            actions = torch.argmax(q_values, dim=1).cpu().numpy() # Exploitation
        ...
        for info in infos:
            if "episode" in info.keys():
                average_10.append(info['episode']['r'])
                print(f"global_step={global_step}, episodic_return={info['episode']['r']}, average_10={statistics.mean(average_10[-10:])}")
                break
        ...
        if global_step > params.learning_starts:
            if global_step % params.train_frequency == 0:
                ...
                with torch.no_grad():
                    ...
                    target_max = target_network(data.next_observations).max(1)[0] #TODOF
                    td_target = data.rewards.flatten() + params.gamma * target_max * (1 - data.dones.flatten()) #TODOF
